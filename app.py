...
def _warmup():
    ...
    try:
        dummy_row  = [0.0] * (len(FEATURES) * 5)  # 80 zeros
        dummy_X    = scaler.transform(np.array([dummy_row], dtype=np.float32))
        _           = model.predict_proba(dummy_X)
        print("[Warmup] ✅ XGBoost pre-warmed — first request will be instant")
